...
- **folkaurixsvc/** – [folkaurixsvc](folkaurixsvc/README.md) console application that captures from the "FolkAurix Mic" device using the WASAPI `IAudioClient` interface and sends the audio to the cloud.
...
3. Run `folkaurixsvc.exe`. The service captures from the "FolkAurix Mic" device using `IAudioClient`. Captured speech is recognised, translated and played back immediately through the default speaker.
